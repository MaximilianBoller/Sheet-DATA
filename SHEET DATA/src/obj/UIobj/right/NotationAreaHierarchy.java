...
public class NotationAreaHierarchy extends JPanel
{
	...
	public NotationAreaHierarchy(int imagearecount, int xPageCoordinate, String name)
	{
		ID = imagearecount;
	...
	//			setBackground(Var.SekColor);
			...
			{
				Var.NotationArea[ID].RequestFocus();
			}
		...
		icon1.addMouseListener(new MouseAdapter()
		{
			public void mousePressed(MouseEvent e)
			{
				...
				if(TextareaVisible)
				{
					Var.NotationArea[ID].setVisible(true);
					icon1.changeIcon("eye.png");
				}
				
				else
				{
					Var.NotationArea[ID].setVisible(false);
		...
		IconLabel icon2 = new IconLabel("HierarchyNotation.png", 20,20,0,0);
