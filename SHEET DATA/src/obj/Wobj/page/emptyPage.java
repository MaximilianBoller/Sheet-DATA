...
public class emptyPage extends JLayeredPane
{	
	...
	public emptyPage(int x, int seitenzahl)
	{
		...
		setBounds(x,0, Var.pageWidth, Var.pageHeight);
		setVisible(true);
	}
		...
		PageMouseListener = new MouseAdapter()
		{
			...
			public void mouseReleased(MouseEvent e)
			{
				...
				if(ReleaseX + ReleaseY > ClickX + ClickY )
				{
					...
					{
						AcTextArea.run("[Text]",Seitenzahl,ReleaseX - ClickX, ReleaseY - ClickY, ClickX, ClickY, 20, Var.standartTypeface, "left");
					}
					...
					if(Var.ActiveTool.contentEquals("notationarea"))
					{
						AcNotationArea.run(1,Seitenzahl,ReleaseX - ClickX, ReleaseY - ClickY, ClickX, ClickY);
					}
				}
				...
				else
				{
					if(Var.ActiveTool.contentEquals("textarea"))
					{
						AcTextArea.run("[Text]",Seitenzahl,ClickX - ReleaseX, ClickY - ReleaseY, ReleaseX, ReleaseY, 20, Var.standartTypeface, "left");
					}
					...
					if(Var.ActiveTool.contentEquals("notationarea"))
					{
						AcNotationArea.run(1,Seitenzahl,ClickX - ReleaseX, ClickY - ReleaseY, ReleaseX, ReleaseY);
					}
				}
